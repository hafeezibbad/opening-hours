...
class DataParsingErrorAssertions:
    def __init__(self, parsing_error_dict: dict):
        self.parsing_errors = parsing_error_dict

    def error_count_is(self, count: int):
        actual_count = len(self.parsing_errors['errors'])
        if actual_count != count:
            error_msg = 'DataParsing error count should have been {expected}, but is actually {actual}'
            pytest.fail(error_msg.format(expected=count, actual=actual_count))

    def has_parsing_error_with_title_and_detail(self, title: str, detail: str):
        for error in self.parsing_errors['errors']:
