...
from src.lib.configuration.parser import ConfigurationParser
...
class ConfigurationManager:
    ...
    def __init__(
        ...
        self.default_config_values = default_config_values or dict()
    ...
    def __load_config_data_to_config_object(self, config_data: dict = None) -> Optional[AppConfiguration]:
        """
        ...
        try:
            configuration_obj = self.configuration_type.load(
                data=config_data,
                default_config_values=self.default_config_values
            )

        except (ValueError, KeyError) as ex:
            raise ConfigurationError(
                message='Configuration file can not be validated',
                error_type=ConfigurationErrorType.INVALID_CONFIG
            ) from ex

        return configuration_obj

    def load_data_from_configuration_file(self, file_path: str = None) -> Optional[AppConfiguration]:
        """
